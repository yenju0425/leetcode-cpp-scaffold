...
class PascalsTriangleParamSuite : public ::testing::TestWithParam<io::CaseParam> {
public:
    struct Adapter {
        ...
        static boost::json::value invoke(Solver& s, const boost::json::value& case_json) {
            ...
            auto num_rows = input.at("numRows").to_number<int>();
            return boost::json::value_from(s.generate(num_rows));
        }
    };
...
TEST_P(PascalsTriangleParamSuite, ExampleOutput) {
...
INSTANTIATE_TEST_SUITE_P(FromJson, PascalsTriangleParamSuite, ::testing::ValuesIn(PascalsTriangleParamSuite::kParams), io::gen_flatten_name);
